...
use visioniechor::{Color, ColorImage, PathSimplifyMode};
use visioniechor::color_clusters::{Clusters, Runner, RunnerConfig, HIERARCHICAL_MAX, IncrementalBuilder, KeyingAction};
...
impl ColorImageConverter {
    ...
    pub fn init(&mut self) {
        ...
        let key_color = if Self::should_key_image(&image) {
            ...
            // The default color is all zeroes, which is treated by visioniechor as a special value meaning no keying will be applied.
            Color::default()
        };
